...
class Editchildformcontroller extends GetxController {
  var iscreate = 0.obs;
  var isread = 0.obs;
  var isupdate = 0.obs;
  var isdelete = 0.obs;
  ...
  Getlistdata() async {
    print('Getlistdata=====searchParams====>>$searchParams');
  ...
  List getItemsWithoutGroup() {
    return childlabellist.where((field) => field['group'] == "" || field['group'] == null ).toList();
  }
  ...
  var formidcode =0.obs;
  Future<void> getchildlist(int recordid,int formid) async {
  ...
  dynamic getInitialValue(String fieldCode) {
    ...
    if (dataMap.containsKey(fieldCode)) {
      ...
      setInitialValue(fieldCode, value.toString());
      if (value is int) {
        setFieldValue(fieldCode, value.toString());
        setInitialValue(fieldCode, value.toString());
  ...
  Future<void> getUiFormDatas(Map<String, dynamic> usecase, String usecaseId,int recordid) async {
    updatedName.value = usecase['collectionName'].replaceAll('.', '/');
    searchParams = {
      'createdBy':UseraccessID.value,
    ...
    try {
      ...
      final response = await helper.postApi(
          "api/v1/${childappCode.value}/${childcode.value}/search;jsessionid=$jsessionid", searchParams);
  ...
  Future<void> deleteListItem(int itemId) async {
    ...
    String url = "$aPPmAINuRL/${updatedName.value}/delete/$itemId;jsessionid=$jsessionid";
  ...
  Future<void> Getchildattributefield(String formId) async {
    ...
    if (res!['success'] == true) {
      ...
      childlabellist.assignAll(filteredList);
      ...
      for (var labellist in childlabellist) {
        if (labellist['code'] == parentKey) {
          setInitialValue(labellist['code']!, recordidcode.value.toString());
          break;
        }
      }
  ...
  Future<void> Getchilditemcode(String formid) async {
    ...
    if (res != null && res['success'] == true) {
      ...
      GetdataList(childappCode.value, formidcode.value.toString());
      update();
    } else {}
  }
    ...
    if (res != null && res['success'] == true) {
      ...
      for (var useCase in useCases) {
        if (result.containsKey(useCase)) {
          ...
          if (data is List) {
            prelaodlist[useCase] = List.from(data); // Assign data to the specific use case
          } else {}
        } else {}
      }
    } else {
  ...
  Future<Map<String, dynamic>?> validateAndSubmitDate(String rule, String dischargeDate) async {
    try {
  ...
  Future<Map<String, dynamic>?> GetUserData(String code,String rule, String admissionId) async {
    try {
              ...
              for (final entry in preloadparentUsecases) {
                ...
                if (value is Map<String, dynamic>) {
                  ...
                  if (nestedValue['id'] == code) {
                    matchingKey = key;
                    processParentFilterAndCallApi(datafields[0].parentFilter, matchingKey!);
                  }
                }
              }
            } else {
  ...
  Future<void> processParentFilterAndCallApi(String parentFilter,
      String matchingKey)
  async {
  ...
  Future<void> getdata(String hrStatus,String filterParams) async {
    prelaodlist.clear();
    try {
      var response = await httpServices.getParentFilterData(appCode: childappCode.value, hrStatus: hrStatus, val: filterParams);
