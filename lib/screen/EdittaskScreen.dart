...
class _AddtaskScreenState extends State<Edittaskscreen> {
  ...
  Edittodocontroller todocontroller = Get.put(Edittodocontroller()) ;
@override
  initState()  {
  ...
  Getinfo() async {
    await todocontroller.GettaskInfo(widget.taskid);

    if (todocontroller.taskdata.value != null) {


      titleController.text = todocontroller.taskdata.value!['title'] ?? '';
      descriptionController.text = todocontroller.taskdata.value!['description'] ?? '';
      dateController.text = todocontroller.taskdata.value!['targetDate'] ?? '';
      String? targetTime = todocontroller.taskdata.value!['targetTime'];


      if (targetTime != null && targetTime.isNotEmpty) {
        // Parse "17:08" or any HH:mm format string
        DateTime parsedTime = DateFormat.Hm().parse(targetTime);

        // Format to 12-hour with AM/PM
        String formattedTime = DateFormat.jm().format(parsedTime); // e.g., 5:08 PM
      ...
      print('taskdata is still null after API call.');
    }
  }
                ...
                Obx(
                  ...
                  () {
                    ...
                    return     TextFormField(
                      style:labelStyle ,
                      controller: titleController, // Set controller
